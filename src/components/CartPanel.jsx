...
export default function CartPanel({ onClose, tableId, restaurantSlug: propSlug }) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Write under restaurants/{restaurantSlug}/orders
      const ordersColRef = collection(db, "restaurants", restaurantSlug, "orders");
      const docRef = await addDoc(ordersColRef, orderData);

      // Navigate to restaurant-scoped checkout
      navigate(`/${restaurantSlug}/checkout`, { state: { orderId: docRef.id } });
      onClose?.();
      clearCart();
      console.log("Order created with ID:", docRef.id);
    } catch (error) {
