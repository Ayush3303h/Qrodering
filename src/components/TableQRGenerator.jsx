...
//     const urls = Array.from({ length: count }, (_, i) => `${base}${slugPart}/?table=${i + 1}`);
//     setQrCodes(urls);
//   };
...
// export default function TableQRGenerator({ baseSlug }) {
//   // default base (no trailing slash)
//   const [baseUrl, setBaseUrl] = useState("https://qrodering.vercel.app");
//   const [numTables, setNumTables] = useState("");
//   const [qrCodes, setQrCodes] = useState([]);
//   const { restaurantSlug: paramSlug } = useParams();
//   // allow passing slug as prop (from Admin) or use URL param if generator is used on a restaurant page
//   const restaurantSlug = baseSlug || paramSlug || "";
...
//   //   {base}/{restaurantSlug}/{tableNumber}
//   // If no restaurantSlug provided, generates: {base}/?table={n} (backwards compatible)
//   const handleGenerate = () => {
//     const count = parseInt(numTables);
//     if (!baseUrl.trim() || isNaN(count) || count <= 0) {
//       alert("Please enter a valid base URL and number of tables");
...
//         ? `${base}${slugPart}/${tableNum}`            // e.g. https://qrodering.vercel.app/streake/1
//         : `${base}/?table=${tableNum}`;              // fallback to old style if no slug
//     });
//     setQrCodes(urls);
//   };
