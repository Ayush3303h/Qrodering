...
export default function OrdersManager({ restaurantSlug: propSlug }) {
  ...
  const handleReject = async (id) => {
    await updateDoc(doc(db, "restaurants", restaurantSlug, "orders", id), { status: "rejected" });
  };

  const handleApprove = async (id) => {
    const time = selectedTime[id] || 10;
    await updateDoc(doc(db, "restaurants", restaurantSlug, "orders", id), {
      status: "approved",
      completionTime: time,
      startTime: serverTimestamp(),
    });
  ...
  const handleComplete = async (id) => {
    await updateDoc(doc(db, "restaurants", restaurantSlug, "orders", id), { status: "completed" });
  };
